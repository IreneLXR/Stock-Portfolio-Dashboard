...
def update_intro(value):
    print(value)
    if value == None:
        return [], "This web application helps you to construct an optimal stock portfolio."
    global num_tickers
    num_tickers = len(value)
    global selected_ticker
    selected_ticker = value
    print(selected_ticker)
    return selected_ticker, "The graph of the historical prices of " + ", ".join(value)
...
def run_monte_carlo(df, num_iter, n):
    ...
    return ret, vol, sharpe, all_weights
...
def draw_ratios(*args, **kwargs):
    ...
    if prop_id == 'select-ticker.value':
        print(prop_id)
        return [], {'display':'none'}, fig_blank(), fig_blank(), [None for i in range(len(ticker_list))]
        ...
        if prop_event_val == 0:
            return [], {'display':'none'}, fig_blank(), fig_blank(), [None for i in range(len(ticker_list))]
                ...
                for i in range(3, len(args)):
                    ...
                    print(args[i])
                    if args[i] != None:
                        ratios[ticker_list[i - 3]] = args[i]
                        sum += int(args[i])
                    else:
                        if ticker_list[i - 3] in selected_ticker and len(selected_ticker) != 1:
                            ratios = {}
                            return  "There's at least one selected stock not receive ratio, cannot calculate portfolio", {'display':'block'}, fig_blank(), fig_blank(), [None for i in range(len(ticker_list))]
                ...
                if sum > 100:
                    ratios = {}
                    return  "Total ratios exceed 100! Please reinput", {'display':'block'}, fig_blank(), fig_blank(), [None for i in range(len(ticker_list))]
                if sum < 100:
                    ratios = {}
                    return "Total ratios are less than 100! Please reinput", {'display':'block'}, fig_blank(), fig_blank(), [None for i in range(len(ticker_list))]
    else:
        # run monte carlo
        for i in range(3, len(args)):
            input_vals.append(args[i])
        ...
        print(log_ret)
        num_iters = 100
        ret, vol, sharpe, weights = run_monte_carlo(log_ret[1:], num_iters, len(selected_ticker))
        res = pd.DataFrame([])
        res['ret'] = ret
        res['vol'] = vol
        res['sharpe'] = sharpe
        ...
        N_PTS = 100
        frontier_rets = np.linspace(res['ret'].min(), res['ret'].max(), N_PTS)
        frontier_vols = []
        ...
        for ret_possible in frontier_rets:
            constraints = ({'type': 'eq', 'fun': check_sum},
                           {'type': 'eq', 'fun': lambda w: get_ret_vol_sharpe(w, log_ret)[0] - ret_possible})
            minimized_res = minimize(minimize_vol, initial_guess, (log_ret[1:],), method='SLSQP', bounds=bounds, constraints=constraints)
    ...
    if prop_id == 'run-monte-carlo.n_clicks':
        #print(res)
        return [], {'display':'none'}, fig, fig_mc, input_vals
    return [], {'display':'none'}, fig, fig_blank(), input_vals
